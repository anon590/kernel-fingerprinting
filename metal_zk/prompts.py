"""Prompts for Metal-ZK kernel generation.

Adjusted from metal-kernels:
- SYSTEM_PROMPT names the ZK regime, calls out Apple-GPU integer/rotate
  quirks (no hardware u64 mul-hi, no native funnel shift), and replaces
  "tolerance" with "bit-exact equality".
- Iteration / stagnation templates are reused unchanged.
"""
...
- `simd_shuffle`, `simd_shuffle_xor`, `simd_broadcast` work for integer
  types (`uint`, `int`, `uint2`, ...) inside a 32-lane simdgroup.
...
You will get structured feedback on compile errors, correctness
failures, and performance, and may iterate on your previous attempt.
""")
...
def detect_stagnation(history: list[dict], best_score: float | None,
    ...
    """Last `window` correct iterations all within `gap` below the
    incumbent, none promoted — the hill-climbing-around-local-optimum
    signature."""
